...
jQuery(function(){
	...
	// HERE GLENN!!! HERE!!! DO YOU SEE ME? I AM HERE!!!
	jQuery(document).ready(function(){
		jQuery( '#it-exchange-paypal-button-substitute' ).css( 'display', 'inline' ).prev( 'form' ).css( 'display', 'none' );
		jQuery( '#it-exchange-paypal-button-substitute' ).click( function() { 
			jQuery( this ).prev( 'form' ).find( 'input[type="image"]' ).click();
		});
		// alert( 'Hello Glenn. I hope you are having a good day.' );
	});
...
function itExchangeSWLogIn(user, pass, remember) {
	jQuery.get( itExchangeSWAjaxURL+'&sw-action=login&sw-un=' + user + '&sw-p1=' + pass + '&sw-remember=' + remember, function(data) {
		if ( '0' === data ) {
...
function itExchangeSWRegister(data) {
	jQuery.get( itExchangeSWAjaxURL+'&sw-action=register&sw-un=' + data.un 
		+ '&sw-fn=' + data.fn
		+ '&sw-ln=' + data.ln
		+ '&sw-em=' + data.em
		+ '&sw-p1=' + data.p1

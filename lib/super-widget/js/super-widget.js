...
jQuery( function( $ ) {
	...
	var $sw = jQuery('.it-exchange-super-widget');

	// Register Clear Cart event
	$sw.on('click', 'a.it-exchange-empty-cart', function(event) {
	...
	$sw.on('submit', 'form.it-exchange-sw-update-cart-quantity', function(event) {
		event.preventDefault();
	...
	$sw.on('click', 'a.it-exchange-checkout-cart', function(event) {
	...
	$sw.on('submit', 'form.it-exchange-sw-buy-now', function(event) {
		event.preventDefault();
	...
	$sw.on('submit', 'form.it-exchange-sw-add-to-cart', function(event) {
		event.preventDefault();
		var quantity         = jQuery(this).children('.product-purchase-quantity').length ? jQuery(this).children('.product-purchase-quantity').val() : 1;
	...
	$sw.on('click', 'a.it-exchange-sw-edit-shipping-method', function(event) {
		event.preventDefault();
	...
	$sw.on('click', 'a.it-exchange-super-widget-shipping-method-cancel-action', function(event) {
		event.preventDefault();
		itExchangeGetSuperWidgetState( 'checkout' );
    ...
    $sw.on('change', '.it-exchange-shipping-method-select', function(event) {
	...
	$sw.on('submit', 'form.it-exchange-sw-update-cart-coupon', function(event) {
	...
	$sw.on('click', 'a.it-exchange-sw-cancel-login-link', function(event) {
		event.preventDefault();
	...
	$sw.on('click', 'a.it-exchange-sw-register-link', function(event) {
		event.preventDefault();
		itExchangeGetSuperWidgetState( 'registration' );
		itExchange.hooks.doAction( 'itExchangeSW.registerLinkClicked' );
	});

	$sw.on('click', 'a.it-exchange-sw-cancel-register-link', function(event) {
		event.preventDefault();
	...
	$sw.on('submit', 'form.it-exchange-sw-log-in', function(event) {
		event.preventDefault();
	...
	$sw.on('submit', 'form.it-exchange-sw-register', function(event) {
		event.preventDefault();
		data = jQuery( ':input', this ).serializeArray();
	...
	$sw.on('click', 'a.it-exchange-shipping-address-requirement-cancel', function(event) {
		event.preventDefault();
	...
	$sw.on('click', 'a.it-exchange-sw-edit-shipping-address', function(event) {
		event.preventDefault();
	...
	$sw.on('submit', 'form.it-exchange-sw-shipping-address', function(event) {
		event.preventDefault();
	...
	$sw.on('click', 'a.it-exchange-billing-address-requirement-cancel', function(event) {
		event.preventDefault();
	...
	$sw.on('click', 'a.it-exchange-sw-edit-billing-address', function(event) {
		event.preventDefault();
		itExchangeGetSuperWidgetState( 'billing-address' );
	});

	// Register Billing Address submit event
	$sw.on('submit', 'form.it-exchange-sw-billing-address', function(event) {
		event.preventDefault();
		data = jQuery( ':input', this ).serializeArray();
	...
	$sw.on('submit', 'form.it-exchange-sw-purchase-dialog', function(event) {
		event.preventDefault();
	...
	$sw.on( 'click', 'a.it-exchange-sw-edit-customer-order-note', function(event) {
		event.preventDefault();
		itExchangeGetSuperWidgetState( 'customer-order-note' );
	});

	// Register the submit event from the edit customer order note view
	$sw.on( 'click', '.it-exchange-customer-order-note-cancel', function(event) {
		event.preventDefault();
		itExchangeGetSuperWidgetState( 'checkout' );
	});

	// Register the cancel event from the edit customer order note view
	$sw.on( 'submit', '.it-exchange-sw-customer-order-note-form', function(event) {
		event.preventDefault();
	...
	$sw.on( 'change', 'input[type=radio][name=saved_address]', function() {
		if ( this.value == 0 ) {
			jQuery( '.it-exchange-sw-address-form' ).show();
		} else {
			jQuery( '.it-exchange-sw-address-form' ).hide();
		}
	} );

	$sw.on( 'click', '.payment-methods-wrapper form[data-type="iframe"] .it-exchange-purchase-button', function ( e ) {
		e.preventDefault();
		...
		$( '.payment-methods-wrapper form, .it-exchange-purchase-dialog-trigger' ).not( $form ).hide();
		$this.hide();
		$selector.show();
	} );

	$sw.on( 'change', '.it-exchange-credit-card-selector input[type="radio"]', function ( e ) {
	...
	$sw.on( 'click', '.it-exchange-checkout-cancel-complete', function ( e ) {
		...
		$( '.payment-methods-wrapper form, .it-exchange-purchase-dialog-trigger' ).show();
...
function itExchangeSWLoadState( state, html ) {
	jQuery('.it-exchange-super-widget').filter(':visible').html(html);
 ...
 * @returns {String}
 */
function itExchangeSWAdditionalFields( fields, additionalBlacklist ) {
	...
	for ( var i =0; i < fields.length; i++ ) {
		...
		if ( typeof field.name != 'undefined' && typeof field.value != 'undefined' && blacklist.indexOf(field) === -1 ) {
...
function itExchangeSWBuyNow( product, quantity, additionalFields ) {
	var additionalFieldsString = itExchangeSWAdditionalFields( additionalFields, [ 'it-exchange-buy-now' ] );
...
function itExchangeSWAddToCart( product, quantity, additionalFields ) {
	...
	var additionalFieldsString = itExchangeSWAdditionalFields( additionalFields, [ 'it-exchange-add-product-to-cart' ] );

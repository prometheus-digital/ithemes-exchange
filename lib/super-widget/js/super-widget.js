...
jQuery(function(){
    ...
    jQuery(document).on('change', '.it-exchange-shipping-method-select', function(event) {
...
function itExchangeInitSWCountryStateSync(countryElement, options) {
	jQuery(countryElement, jQuery('.it-exchange-super-widget').filter(':visible')).itCountryStatesSync(options);
}

...
jQuery(function(){
	...
	jQuery(document).on('change', '.it-exchange-super-widget input.product-cart-quantity', function(event) {
		event.preventDefault();
		var product  = jQuery( this ).data('cart-product-id');
	...
	jQuery(document).on('click', '.it-exchange-super-widget input.it-exchange-update-quantity-button', function(event) {
		event.preventDefault();
		jQuery( '.it-exchange-super-widget input.product-cart-quantity' ).each( function() {
			var product  = jQuery( this ).data('cart-product-id');
			var quantity = jQuery( this ).val();
			itExchangeSWUpdateQuantity(product, quantity);
		});
...
function itExchangeSWRemoveItemFromCart( product ) {
	jQuery.get( itExchangeSWAjaxURL+'&no_debug=1&sw-action=remove-from-cart&sw-cart-product=' + product, function(data) {
		if ( itExchangeSWMultiItemCart ) {
			itExchangeGetSuperWidgetState( 'product', itExchangeSWOnProductPage );
		} else {
			if ( itExchangeIsUserLoggedIn )
...
function itExchangeSWUpdateQuantity(product, quantity) {
	jQuery.get( itExchangeSWAjaxURL+'&no_debug=1&sw-action=update-quantity&sw-cart-product=' + product + '&sw-quantity=' + quantity );
}

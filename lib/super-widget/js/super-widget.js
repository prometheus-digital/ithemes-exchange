...
function itExchangeGetSuperWidgetState( state, product, focus ) {
	...
	jQuery.get( itExchangeSWAjaxURL+'&sw-action=get-state&state=' + state + productArg + focusArg, function(data) {
		...
		if ( 'billing-address' == state ) {
			...
			itExchangeInitSWCountryStateSync(billingSyncOptions);
		}
	});
...
*/
function itExchangeInitSWCountryStateSync(options) {
	jQuery('#it-exchange-billing-address-country', jQuery('.it-exchange-super-widget').filter(':visible')).itCountryStatesSync(options);
}

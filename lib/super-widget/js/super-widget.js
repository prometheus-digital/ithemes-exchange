...
jQuery(function(){
	...
	jQuery( '.it-exchange-super-widget' ).on('submit', 'form.it-exchange-sw-purchase-dialog', function(event) {
		...
		var deferred = jQuery.Deferred(),
			method = jQuery( 'input[name="it-exchange-transaction-method"]', $this ).val(),
		...
		} else if ( itExchange.hooks.hasAction( 'itExchangeSW.preSubmitPurchaseDialog' ) ) {
			itExchange.hooks.doAction( 'itExchangeSW.preSubmitPurchaseDialog', method, $this, deferred );
		} else {
			deferred.resolve();
		}
		...
		deferred.done( function ( data ) {
			var submit = typeof data === 'undefined' ? jQuery( ':input', $this ).serializeArray() : data;
			itExchangeSWSubmitPurchaseDialog( submit );
		} ).fail( function() {

(function ( api, Common, $, _, APIConfig, ProfileConfig ) {
		...
		initialize: function ( options ) {
			...
			if ( b = this.model.billingAddress() ) {
				this.views.add(
					...
					this.billingView = new app.Views.ManageAddressesAddress( {
						model : b,
						type  : 'billing',
						parent: this,
					} )
			...
			if ( s = this.model.shippingAddress() ) {
				this.views.add(
					'.it-exchange-customer-addresses-container',
					this.shippingView = new app.Views.ManageAddressesAddress( {
						model : s,
						type  : 'shipping',
						parent: this,
					} )
	...
	app.Views.ManageAddressesAddress = api.View.extend( {
		...
		initialize: function ( options ) {
			...
			if ( !this.type && this.model.parentModel ) {
				...
				callback: ( function ( key ) {
					this.model.parentModel.save( key + '_address', this.model.id, {
						success: (function () {
							...
							var currentView = this.parent[key + 'View'];
							this.parent[key + 'View'] = this;

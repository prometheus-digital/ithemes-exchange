...
function itExchangeInitShippingJSForCheckout() {
	...
	jQuery(document).on( 'change', '.it-exchange-checkout-shipping-address-purchase-requirement input[type=radio][name=saved_address]', function() {
		if ( this.value == 0 ) {
			jQuery( '.it-exchange-checkout-address-form--shipping' ).show();
		} else {
			jQuery( '.it-exchange-checkout-address-form--shipping' ).hide();
		}
	} );
}
...
function itExchangeInitShippingJSForCheckoutMethodHandler() {
	...
	jQuery( '#it-exchange-cart .it-exchange-shipping-method-select' ).unbind();
	jQuery( document ).on( 'change', '#it-exchange-cart .it-exchange-shipping-method-select', function ( event ) {
		...
		jQuery.post( ITExchangeCheckoutRefreshAjaxURL, { 'shipping-method': value }, function ( response ) {
			if ( response ) {
				jQuery( '#it-exchange-cart' ).replaceWith( response );
	...
	jQuery( '.it-exchange-multiple-shipping-methods-select' ).unbind();

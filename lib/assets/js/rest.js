(function ( Schemas, ExchangeCommon, $, _, Backbone, wp, Config ) {
	...
	app.Views.DefaultPurchaseMethod = app.View.extend( {
		...
		initialize: function ( options ) {
			...
			if ( this.model.accepts( 'token' ) ) {
				this.primaryToken = this.model.get( 'method.primaryToken' );
			}
		},
		...
		render: function () {
			...
			if ( this.primaryToken ) {
				var tokenView = new app.Views.PaymentTokenSelectorToken( {
					model  : new app.Models.PaymentToken( this.primaryToken ),
					count  : 0,
					tagName: 'div',
				} );
				this.views.add( '.it-exchange-checkout-default-method--source', tokenView );

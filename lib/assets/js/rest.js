(function ( Schemas, ExchangeCommon, $, _, Backbone, wp, Config ) {
	...
	app.Views.PaymentTokensManagerToken = app.View.extend( {
		...
		render: function () {
			...
			if ( $.payment && this.model.get( 'expiration.editable' ) ) {
		...
		onSaveClicked: function ( e ) {
			...
			this.model.set( 'label', this.$( '.it-exchange-manage-tokens-token-edit--label-input' ).val() );

			var expiration = this.$( '.it-exchange-manage-tokens-token-edit--expiration-input' ).val();

			if ( expiration ) {
				var parts = expiration.split( '/' );
				this.model.set( 'expiration.month', parts[0].trim() );
				this.model.set( 'expiration.year', parts[1].trim() );
			}

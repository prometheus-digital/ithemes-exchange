(function ( Schemas, ExchangeCommon, $, _, Backbone, wp, Config ) {
	...
	app.Model = Backbone.DeepModel.extend( {
			...
			if ( options.embed ) {
				...
				options.success = function ( model, response, options ) {
					...
					_.each( embeds, function ( data, slug ) {
						...
						if ( _.isString( val ) ) {
							model[key] = new (ExchangeCommon.getFunctionByName( val, window ))( data, { parent: model } );
						...
						if ( val.prototype instanceof Backbone.Model ) {
							model[key] = new val( data, { parent: model } );
						} else if ( val.prototype instanceof Backbone.Collection ) {
							model[key] = new val( data, { parent: model } );
						} else {
							model[key] = val( data, model );
						}
					} );

					if ( success ) {
						return success.call( options.context, model, response, options )
					}
				};
			}
		...
		fetchIfEmpty: function ( options ) {
			...
			if ( this.attributes.length > 1 && !this.isNew() ) {
				return $.when();
			}
	...
	app.Collection = Backbone.Collection.extend( {
		...
		doFilter: function ( filters, options ) {
			...
			if ( options.local && options.reset && this.models.length > 0 ) {
				var where = {};
	...
	app.Models.Transaction = app.Model.extend( {
		...
		tags         : [],
		_refunds     : 'ITExchangeAPI.Collections.Refunds',
		_customer    : 'ITExchangeAPI.Models.Customer',
		_paymentToken: 'ITExchangeAPI.Models.PaymentToken',
		 ...
		 */
		refunds: function () {
			if ( !this._refunds instanceof app.Collections.Refunds ) {
		...
		customer: function () {
			...
			if ( !this._customer instanceof app.Models.Customer ) {
		...
		paymentToken: function () {
			...
			if ( !this._paymentToken instanceof app.Models.PaymentToken ) {
	...
	app.Models.Customer = app.Model.extend( {
		...
		_tokens: 'ITExchangeAPI.Collections.PaymentTokens',
		 ...
		 */
		tokens: function () {
			if ( !this._tokens instanceof app.Collections.PaymentTokens ) {
	...
	app.Collections.Products = app.Collection.extend( {
	...
	app.Models.Subscription = app.Model.extend( {
		...
		beneficiary: function () {
			...
			if ( !this._beneficiary instanceof app.Models.Customer ) {
				this._beneficiary = new app.Models.Customer( {
		...
		transaction: function () {
			...
			if ( !this._transaction instanceof app.Models.Transaction ) {
				this._transaction = new app.Models.Transaction( {
		 ...
		 *
		 * @param [options] {*}
		 * @param [options.paused_by] {int|app.Models.Customer} Specify who paused the subscription.
		 *
		 * @returns {Promise}
		 */
		pause: function ( options ) {
			...
			if ( options.paused_by ) {
				...
				if ( options.paused_by instanceof app.Models.Customer ) {
		 ...
		 *
		 * @param [options] {*}
		 * @param [options.resumed_by] {int|app.Models.Customer} Specify who resumed the subscription.
		 *
		 * @returns {Promise}
		 */
		resume: function ( options ) {
			...
			if ( options.resumed_by ) {
				...
				if ( options.resumed_by instanceof app.Models.Customer ) {
		 ...
		 *
		 * @param [cart] {app.Models.Cart} Optional. If empty, will create a cart transparently.
		 * @param [options] {*}
		 *
				...
				success: function ( cart_item ) {
					...
					if ( cart ) {
						...
						options.success = function ( model ) {
							...
							if ( success ) {
								r = success.apply( options.context, arguments );
							}
	...
	app.loadCart = function ( create, email ) {
		...
		app.Models.Cart = app.Model.extend( {
			...
			sortedItems     : {},
			_allItems       : null,
			_purchaseMethods: 'ITExchangeAPI.Collections.PurchaseMethods',
			_customer       : 'ITExchangeAPI.Models.Customer',
			guestEmail      : '',
			...
			updateItems: function () {
				...
				var items = this.get( 'items' ), item, itemModel, collection;
	...
	app.Views.Checkout = app.View.extend( {
		...
		events: {
			...
			'click .it-exchange-checkout-cancel-purchase-method'      : 'onCancelMethod',
		},
	...
	app.Views.PurchaseMethod = app.View.extend( {
		...
		onMethodSelected: function ( e ) {
			...
			this.addView( new app.Views.CompletePurchaseMethod( {
		...
		onPurchaseFinished: function ( transaction, $target ) {
			$target.removeAttr( 'disabled' );
			$target.text( i18n.checkout.purchased );
		},
	} );
	...
	app.Views.CompletePurchaseMethod = app.View.extend( {
		...
		cancel: function () {
			this.purchaseMethodView.removeView( this );
		},
	...
	app.Views.PaymentTokenSelectorToken = app.View.extend( {
		...
		render: function () {
			...
			if ( this.model.get( 'primary' ) && ( this.parent.$el.is( ':hidden' ) || $( 'input[name="' + attr.inputName + '"]:checked' ).length < 1) ) {
	...
	app.Views.PaymentTokensManager = app.View.extend( {
		...
		initialize: function () {
			...
			var gateways = this.collection.groupBy( 'gateway' );

			_.each( gateways, function ( tokens, gateway ) {

				if ( !this.gatewayClasses[gateway] ) {
					this.gatewayClasses[gateway] = 'it-exchange-manage-tokens-list-' + gateway;
				}

				_.each( tokens, function ( token ) {
					this.views.add(
						'.' + this.gatewayClasses[gateway],
		...
		render: function () {
			...
			_.each( this.gatewayClasses, function ( className, gateway ) {
				this.$( '.it-exchange-manage-tokens-list-container' ).append(
					'<h5>' + gateway + '</h5><ul class="it-exchange-manage-tokens-list ' + className + '"></ul>'
				);
			}, this );
	...
	app.Views.PaymentTokensManagerToken = app.View.extend( {
		...
		onEditClicked: function ( e ) {
			this.$( '.it-exchange-manage-token-label' ).hide();
			this.$( '.it-exchange-manage-token-edit-container' ).show();

			this.$( '.it-exchange-manage-tokens-token-action--edit' ).hide();
			this.$( '.it-exchange-manage-tokens-token-action--delete' ).hide();
			this.$( '.it-exchange-manage-tokens-token-action--primary' ).hide();
			this.$( '.it-exchange-manage-tokens-token-action--save' ).show();
			this.$( '.it-exchange-manage-tokens-token-action--cancel' ).show();
		},

		onCancelClicked: function ( e ) {
			this.$( '.it-exchange-manage-token-label' ).show();
			this.$( '.it-exchange-manage-token-edit-container' ).hide();

			this.$( '.it-exchange-manage-tokens-token-action--edit' ).show();
			this.$( '.it-exchange-manage-tokens-token-action--delete' ).show();
			this.$( '.it-exchange-manage-tokens-token-action--primary' ).show();
			this.$( '.it-exchange-manage-tokens-token-action--save' ).hide();
			this.$( '.it-exchange-manage-tokens-token-action--cancel' ).hide();
		},
...
})( window.ITExchangeRESTSchemas || {}, window.ExchangeCommon, jQuery, window._, window.Backbone, window.wp, window.ITExchangeRESTConfig );

(function ( Schemas, ExchangeCommon, $, _, Backbone, wp, Config ) {
	...
	var app = {
		...
		sync: function ( method, model, options ) {
			...
			if ( method !== 'read' && app.Models.CartItem && model instanceof app.Models.CartItem ) {
	...
	app.Models.Subscription = app.Model.extend( {
		...
		renew: function ( cart ) {
			...
			$.ajax( {
				...
				success: function ( cart_item, _, xhr ) {

					if ( cart ) {
						cart.products().add( cart_item );
						cart.fetch(); // Update total lines

						deferred.resolve( cart );
					} else {
						cart = new app.Models.Cart( {
							id: xhr.getResponseHeader( 'X-Cart-ID' ),
						} );

(function ( Schemas, ExchangeCommon, $, _, Backbone, wp, i18n ) {
	"use strict";
	...
	app.Views.CompletePurchaseMethod = app.View.extend( {
		...
		render: function () {
			this.$el.text( i18n.checkout.completePurchase );
		},
	...
	app.Views.PaymentTokenSelector = app.View.extend( {
		...
		addPaymentTokenView: function ( paymentTokenModel ) {
			...
			if ( this.showAddNew && this.$el.is( ':hidden' ) ) {
...
})( window.ITExchangeRESTSchemas || {}, window.ExchangeCommon, jQuery, window._, window.Backbone, window.wp, window.ITExchangeRESTi18n );

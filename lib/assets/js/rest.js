(function ( Schemas, ExchangeCommon, $, _, Backbone, wp, Config ) {
	...
	app.loadCart = function ( create, email ) {
		...
		app.Models.Cart = app.Model.extend( {
			...
			updateTotalLines: function () {
				this.lineItemTotals().set( this.get( 'total_lines' ) );
			},
	...
	app.Views.PaymentTokenSelector = app.View.extend( {
		...
		onChange: function ( e ) {
			...
			this.$( '.it-exchange-payment-token-selected' ).removeClass( 'it-exchange-payment-token-selected' );

			this.$( e.currentTarget ).parent().parent().addClass( 'it-exchange-payment-token-selected' );

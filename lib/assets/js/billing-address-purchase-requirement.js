...
function itExchangeInitBillingAddressJS() {
	...
	jQuery(document).on( 'change', '.it-exchange-checkout-billing-address-purchase-requirement input[type=radio][name=saved_address]', function() {
		if ( this.value == 0 ) {
			jQuery( '.it-exchange-checkout-address-form--billing' ).show();
		} else {
			jQuery( '.it-exchange-checkout-address-form--billing' ).hide();
		}
	} );

<?php
/**
 * CartBuddy Transactions Post Type class.
 *
 * Sets up the transactions post type.
 *
 * @package IT_CartBuddy
 * @since 0.1
*/

if ( ! class_exists( 'IT_CartBuddy_Transactions' ) ) {
	...
	class IT_CartBuddy_Transactions extends ITPostType {
		...
		*/
		var $_settings = array(
			'rewrite'             => array(
				'slug' => 'post-type-cartbuddy-transaction',
			),
		...
		*/
		 ...
		 * @since 0.1
		*/
		function IT_CartBuddy_Transactions( &$parent ) {
			$this->_parent = &$parent;
			IT_CartBuddy_Transactions::ITPostType();
			if ( is_admin() ) {
				$this->register_meta_boxes();
				$this->register_admin_pages();
				if ( ! empty( $_GET['post_type'] ) && 'it_cb_transaction' == $_GET['post_type'] && ! empty( $_GET['page'] ) && in_array( $_GET['page'], array_keys( $this->_menu_pages ) ) ) {
		...
		*/
		function register_meta_boxes() {
			$meta_boxes = apply_filters( 'it_cartbuddy-transaction_metaboxes', array() );
		...
		*/
		function register_admin_pages() {
			...
			// Add pages
			$pages = apply_filters( 'it_cartbuddy-transaction_admin_pages', $pages );
		...
		*/
		function register_admin_page_tabs() {
			$this->_page_tabs = apply_filters( 'it_cartbuddy-transaction_admin_tabs', array() ); 
		}
	...
	new IT_CartBuddy_Transactions( &$this );
}

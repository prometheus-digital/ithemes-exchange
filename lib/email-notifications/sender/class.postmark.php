...
class IT_Exchange_Email_Postmark_Sender implements IT_Exchange_Email_Sender {
	...
	public function send( IT_Exchange_Sendable $email ) {
		return $this->make_api_request( 'email', $this->convert_sendable_to_api_format( $email ) );
	}
	...
	protected function make_api_request( $endpoint, $data, IT_Exchange_Sendable $sendable = null ) {
		...
		switch ( $code ) {
			...
			case 500:
			case 503:
				throw new IT_Exchange_Email_Delivery_Exception( 'A temporary problem occured with Postmark\'s servers. Try again later.' );
		}

...
*/
	...
	define( 'EXCHANGEWP_STORE_URL', 'https://exchangewp.com' ); // you should use your own CONSTANT name, and be sure to replace it throughout this file

	// the name of your product. This should match the download name in EDD exactly
	define( 'EXCHANGEWP_ITEM_NAME', 'ExchangeWP' ); // you should use your own CONSTANT name, and be sure to replace it throughout this file

	// the name of the settings page for the license input to be displayed
	define( 'EXCHANGEWP_PLUGIN_LICENSE_PAGE', 'exchangewp-license' );
	...
	function exchangewp_register_option() {
		// creates our settings in the options table
		register_setting('exchangewp_license_key', 'exchangewp_license_key', 'exchangewp_sanitize_license' );
	}

	function exchangewp_sanitize_license( $new ) {
		$old = get_option( 'exchangewp_license_key' );
		if( $old && $old != $new ) {
			delete_option( 'exchangewp_license_status' ); // new license has been entered, so must reactivate
		}
		return $new;
	}
	...
	*************************************/
	...
	function exhangewp_activate_license() {

		// listen for our activate button to be clicked
		if( isset( $_POST['exchangewp_license_activate'] ) ) {

			// run a quick security check
		 	if( ! check_admin_referer( 'exchangewp_nonce', 'exchangewp_nonce' ) )
				return; // get out if we didn't click the Activate button

			// retrieve the license from the database
			$license = trim( get_option( 'exchangewp_license_key' ) );
			...
			if ( ! empty( $message ) ) {
				$base_url = admin_url( 'admin.php?page=it-exchange-settings&tab=license' );
			...
			wp_redirect( admin_url( 'admin.php?page=it-exchange-settings&tab=license' ) );
			exit();
		}
	}
	...
	***********************************************/
	...
	function exchangewp_deactivate_license() {
	...
	*************************************/
	 ...
	 */
	function exchangewp_admin_notices() {

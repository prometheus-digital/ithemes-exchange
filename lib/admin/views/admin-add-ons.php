...
<div class="wrap">
	...
	if ( $enabled = it_cart_buddy_get_option( 'cart_buddy_enabled_add_ons' ) ) { 
		foreach( (array) $enabled as $slug => $location ) { 
			...
			echo '<p style="margin-left:60px;text-align:center;width:75px;background:#999;border:1px solid #777;padding:2px;"><a href="' . wp_nonce_url( get_site_url() . '/wp-admin/admin.php?page=it-cart-buddy-addons&it-cart-buddy-disable-addon=' . $slug, 'cart-buddy-disable-add-on' ) . '" style="text-decoration:none;color:#fff;">Disable</a></p></div>';
		}   
	} else {

	...
	$( document ).ready( function ( $ ) {
		...
		$( ".upgrade-row button" ).click( function ( e ) {
			...
			beginPromise.done( function ( upgrade ) {
				...
				var $feedback = $( 'textarea', $row );

				var initial = '## Beginning Upgrade ##\r\n';
				initial += 'Slug: ' + upgrade.slug + '\r\n';
				initial += 'Items: ' + upgrade.totalItemsToUpgrade + '\r\n';
				initial += 'Rate: ' + upgrade.rate + '\r\n';

				$feedback.val( initial );
	...
	function initializeLoop( upgrade, $progress, $feedback ) {
		...
		$( upgrade ).on( 'it-exchange.step-completed', function ( event, step ) {
			...
			if ( ! upgrade.completed() && ! upgrade.halted ) {
				...
				$feedback.val( $feedback.val() + 'Time: ' + executionTime + 's' );

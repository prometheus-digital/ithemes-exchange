(function ( $, api, Common ) {
	...
	$( document ).ready( function () {
		...
		var transaction = new api.Models.Transaction( EXCHANGE.transaction, {
		...
		$( '#it-exchange-update-transaction-status' ).on( 'change', function () {
			...
			transaction.save( { 'status.slug': $selected.val() }, {
				success: function () {
					...
					$( '#it-exchange-update-transaction-status-success' ).animate( {
						opacity: '1'
					}, 500, function () {
					...
					updateStatusLabel( transaction.get( 'status.label' ) );
		...
		$( "#add-refund" ).click( function ( e ) {
			...
			var refund = transaction.refunds().create( { amount: ExchangeCommon.getFloatFromString( $refund.val() ) }, {
				success: function () {
					$refund.val( '' );
					...
					var refundLabel = EXCHANGE.refundLabel;
...
})( jQuery, ITExchangeAPI, ExchangeCommon );

...
//			$('#it-exchange-gallery-images #' + ui.draggable.context.id ).remove();
	...
	*/	
	...
	$( '#it-exchange-product-downloads').on( 'click', '.download-remove a', function( event ) {
		...
		$( it_exchange_new_item_clone ).removeClass('hidden download-item-clone').addClass( '.download-item-' + it_exchange_new_download_interation ).find( '.download-name input' ).attr( 'name', 'it-exchange-digital-downloads-new[' + it_exchange_new_download_interation + '][name]' ).find( '.downloads-source input' ).attr( 'name', 'it-exchange-digital-downloads[' + it_exchange_new_download_interation + '][source]' );

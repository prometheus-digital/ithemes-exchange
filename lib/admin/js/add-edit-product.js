...
/*	$('.categorydiv').each( function(){
		...
		$( '#it-exchange-add-edit-product-interface-main' ).on( 'click', '#' + taxonomy + '-add-submit', function(){
			$('#new' + taxonomy).focus();
		});
	...
	*/
	...
	$( '#it_exchange_advanced-sortables > div' ).each( function() {
		if ( $( this ).attr( 'id' ) != 'it-exchange-advanced-tabs' ) {
			...
			$( '#it-exchange-advanced-tab-nav' ).append( '<li><a href="#' + id + '">' + $( this ).find( '.hndle span' ).text() + '</a></li>' );

			var existing = $(this).find( '.inside').detach();
			$( '#it-exchange-advanced-tabs' ).append( '<div id="' + id + '"><div class="inner"></div></div>');

			existing.appendTo('#' + id + ' .inner');

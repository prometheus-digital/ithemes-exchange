jQuery(document).ready(function($) {
	...
	$('.categorydiv').each( function(){
		...
		$( '#it-exchange-add-edit-product-interface-main' ).on( 'click', '#' + taxonomy + '-tabs a', function(){
		...
		$( '#it-exchange-add-edit-product-interface-main' ).on( 'keypress', '#new' + taxonomy, function(event){
			if( 13 === event.keyCode ) {
				event.preventDefault();
				$('#' + taxonomy + '-add-submit').click();
			}
		});
		$( '#it-exchange-add-edit-product-interface-main' ).on( 'click', '#' + taxonomy + '-add-submit', function(){ $('#new' + taxonomy).focus(); });
		...
		$( '#it-exchange-add-edit-product-interface-main' ).on( 'click', '#' + taxonomy + '-add-toggle', function() {
		...
		$( '#it-exchange-add-edit-product-interface-main' ).on( 'click', '#' + taxonomy + 'checklist-pop > li.popular-category > label input[type="checkbox"], #' + taxonomy + 'checklist > li.popular-category > label input[type="checkbox"]', function() {
	...
	*/
	$( '#it-exchange-product-images-container' ).on('click', '.it-exchange-gallery-images li .remove-item', function() {
	...
	*/
	$( '#it-exchange-product-images-container' ).on( 'click', '.it-exchange-feature-images-div .remove-item', function() {
	...
	*/
	...
	// Sale Scheduling
	$("#it-exchange-enable-sale-schedule-start").click(function() {
		if ( $(this).is(':checked') ) {
			$("#it-exchange-sale-schedule-start-container").show();
		} else {
			$("#it-exchange-sale-schedule-start-container").hide();
		}
	});

	$("#it-exchange-enable-sale-schedule-end").click(function() {
		if ( $(this).is(':checked') ) {
			$("#it-exchange-sale-schedule-end-container").show();
		} else {
			$("#it-exchange-sale-schedule-end-container").hide();
		}
	});

	$( '#it-exchange-sale-schedule-start' ).datepicker({
		prevText: '',
		nextText: '',
		minDate: 0,
		dateFormat: $( 'input[name="it-exchange-sale-schedule-df"]' ).val(),
		onSelect: function(date) {
			$("#it-exchange-sale-schedule-end").datepicker( 'option', 'minDate', date );
		}
	});
	...
	$( '#sale-price' ).on( 'focusout', function() {
		if ( $( this ).data( 'symbol-position') == 'before' )
			$( this ).val( $( this ).data( 'symbol') + it_exchange_number_format( $( this ).val(), 2, $( this ).data( 'decimals-separator' ), $( this ).data( 'thousands-separator' ) ) );
		else
			$( this ).val( it_exchange_number_format( $( this ).val(), 2, $( this ).data( 'decimals-separator' ), $( this ).data( 'thousands-separator' ) ) + $( this ).data( 'symbol' ) );
	...
	// Move hidden view store link to notification if it is present
	var $viewStoreLink = $('.it-exchange-view-store-on-update-link').children('a');
	if ( $viewStoreLink.length > 0 )
		$('#message').find('a').after($viewStoreLink ).after( ' | ' );
});

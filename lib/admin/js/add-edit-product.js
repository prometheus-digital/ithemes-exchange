...
//			$('#it-exchange-gallery-images #' + ui.draggable.context.id ).remove();
	...
	*/	
	...
	$( '#it-exchange-advanced-tabs-toggle' ).on( 'click', function( event ) {
		...
		if ( $( this ).hasClass( 'advanced-hidden' ) ) {
//			$( this ).text( $( this ).attr( 'data-visible') );
			$( '#it-exchange-advanced-tabs' ).slideDown().animate({opacity:1},250);
		} else {
//			$( this ).text( $( this ).attr( 'data-hidden') );
...
//	var it_exchange_new_download_interation = 0;
	...
	$( '#it-exchange-product-downloads').on( 'click', '.download-remove a', function( event ) {
		...
		$( it_exchange_new_item_clone ).removeClass('hidden download-item-clone').find( '.download-name input' ).attr( 'name', 'it-exchange-digital-downloads[' + it_exchange_new_download_interation + '][name]' ).find( '.downloads-source input' ).attr( 'name', 'it-exchange-digital-downloads[' + it_exchange_new_download_interation + '][source]' );

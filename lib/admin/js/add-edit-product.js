jQuery(function(){
	...
	it_exchange_add_images_frame = {
		...
		select: function() {
			...
			var feature_image_set = jQuery('#it-exchange-feature-image input').length;
			
			var current_image_count = jQuery('#it-exchange-gallery-images li input').length + feature_image_set;
			...
			jQuery( selected_images ).each( function( i, image ) {
				...
				if ( feature_image_set == 0 ) {
					jQuery('#it-exchange-feature-image').append('<a href id="' + image.id + '" class="image-edit"><img src=" ' + data_image_sizes[0] + '" data-large="' + data_image_sizes[0] + '" data-thumb="' + data_image_sizes[1] + '" alt="' + image.alt + '" /><input type="hidden" name="it-exchange-image-id[' + current_image_count + ']" value="' + image.id + '" /></a>');
					feature_image_set = 1;
				} 
				else {
					jQuery('#it-exchange-add-new-image').before('<li><a href id="' + image.id + '" class="image-edit"><img src=" ' + data_image_sizes[1] + '" data-large="' + data_image_sizes[0] + '" data-thumb="' + data_image_sizes[1] + '" alt="' + image.alt + '" /><input type="hidden" name="it-exchange-image-id[' + current_image_count + ']" value="' + image.id + '" /></a></li>');
				}
			...
			if ( feature_image_set == 1 )
				jQuery('#it-exchange-add-new-image').removeClass('empty');
	...
	it_exchange_edit_image_frame = {
		...
		init: function() {
			jQuery('#wpbody').on('click', '#it-exchange-gallery-images li .image-edit', function(e) {
				e.preventDefault();
	...
	it_exchange_gallery_sortable = {
		...
		revert: true,
		start: function( e, ui ) {
			jQuery('.sorting-placeholder').html(ui.item.context.innerHTML);
...
//			console.log(ui.item.context.innerHTML);
	...
	it_exchange_feature_droppable = {
		...
		drop: function( e, ui ) {
			jQuery('.sorting-placeholder').remove();
			jQuery(this).removeClass('over');

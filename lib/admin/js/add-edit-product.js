		...
		select: function() {
			...
			$( selected_images ).each( function( i, image ) {
				...
				if ( feature_image_set == 0 ) {
					$('#it-exchange-feature-image .feature-image').append('<li id="' + itExchangeGenerateUniqueJSID() + '"><a href class="image-edit"><img src=" ' + data_image_sizes[0] + '" data-large="' + data_image_sizes[0] + '" data-thumb="' + data_image_sizes[1] + '" alt="' + image.alt + '" /><span class="overlay"></span></a><span class="remove-item">&times;</span><input type="hidden" name="it-exchange-product-images[' + current_image_count + ']" value="' + image.id + '" /></li>');
					feature_image_set = 1;
				}
				else {
					$('#it-exchange-add-new-image').before('<li id="' + itExchangeGenerateUniqueJSID() + '"><a href class="image-edit"><img src=" ' + data_image_sizes[1] + '" data-large="' + data_image_sizes[0] + '" data-thumb="' + data_image_sizes[1] + '" alt="' + image.alt + '" /><span class="overlay"></span></a><span class="remove-item">&times;</span><input type="hidden" name="it-exchange-product-images[' + current_image_count + ']" value="' + image.id + '" /></li>');
				}
	...
	*/
	it_exchange_edit_image_frame = {
		frame: function() {
			...
			this._frame.on( 'open', this.open ).on('open', this.setSelection).state('library').on( 'select', this.select );
		...
		select: function() {
			var selected_images  = this.get('selection').toJSON();
			var data_image_sizes = new Array();
			...
			$( selected_images ).each( function( i, image ) {
				...
				var $imageDiv = $('.image-edit', '#it-exchange-product-images li[id="' + it_exchange_edit_image_frame.liId + '"]');

				// Update the li for the clicked image with the new image's data
				$imageDiv.find('img').attr('src', data_image_sizes[ $imageDiv.hasClass('is-featured') ? 0 : 1]).attr('data-large', data_image_sizes[0]).attr('data-thumb', data_image_sizes[1]).attr('alt',image.alt);

				// Update teh input for the clicked image with the new image's id
				$('#it-exchange-product-images li[id="' + it_exchange_edit_image_frame.liId + '"]').find('input').attr('value',image.id).end().attr('data-image-id',image.id).data('image-id',image.id);
			});
		},

		...
		select: function() {
			...
			$( selected_images ).each( function( i, image ) {
				...
				if ( feature_image_set == 0 ) {
					$('#it-exchange-feature-image .feature-image').append('<a href id="' + image.id + '" class="image-edit"><img src=" ' + data_image_sizes[0] + '" data-large="' + data_image_sizes[0] + '" data-thumb="' + data_image_sizes[1] + '" alt="' + image.alt + '" /></a><input type="hidden" name="it-exchange-image-id[' + current_image_count + ']" value="' + image.id + '" />');
					feature_image_set = 1;
				}
				else {
					$('#it-exchange-add-new-image').before('<li><a href id="' + image.id + '" class="image-edit"><img src=" ' + data_image_sizes[1] + '" data-large="' + data_image_sizes[0] + '" data-thumb="' + data_image_sizes[1] + '" alt="' + image.alt + '" /></a><span class="remove-item">&times;</span><input type="hidden" name="it-exchange-image-id[' + current_image_count + ']" value="' + image.id + '" /></li>');
				}

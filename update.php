...
function edd_sample_license_menu() {
	add_plugins_page( 'ExchangeWP License', 'ExchangeWP License', 'manage_options', EXCHANGEWP_PLUGIN_LICENSE_PAGE, 'edd_sample_license_page' );
}
...
function edd_sample_activate_license() {
	...
	if( isset( $_POST['edd_license_activate'] ) ) {
		...
		$api_params = array(
			...
			'license'    => $license,
			'item_name'  => urlencode( EXCHANGEWP_ITEM_NAME ), // the name of our product in EDD
			'url'        => home_url()
		);

		// Call the custom API.
		$response = wp_remote_post( EXCHANGEWP_STORE_URL, array( 'timeout' => 15, 'sslverify' => false, 'body' => $api_params ) );
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'item_name_mismatch' :
						...
						$message = sprintf( __( 'This appears to be an invalid license key for %s.' ), EXCHANGEWP_ITEM_NAME );
						break;
		...
		if ( ! empty( $message ) ) {
			$base_url = admin_url( 'plugins.php?page=' . EXCHANGEWP_PLUGIN_LICENSE_PAGE );
		...
		wp_redirect( admin_url( 'plugins.php?page=' . EXCHANGEWP_PLUGIN_LICENSE_PAGE ) );
		exit();
	}
}
...
function edd_sample_check_license() {
	...
	$api_params = array(
		...
		'license' => $license,
		'item_name' => urlencode( EXCHANGEWP_ITEM_NAME ),
		'url'       => home_url()
	);

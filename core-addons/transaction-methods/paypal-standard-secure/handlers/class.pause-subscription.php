...
class ITE_PayPal_Standard_Secure_Pause_Subscription_Handler implements ITE_Gateway_Request_Handler {
	...
	public function handle( $request ) {
		...
		if ( is_wp_error( $response ) ) {
			it_exchange_log( 'Network error while pausing PayPal Secure subscription: {error}', ITE_Log_Levels::WARNING, array(
				'_group' => 'subscription',
		...
		if ( ! empty( $response_array['ACK'] ) && $response_array['ACK'] === 'Success' ) {
			...
			it_exchange_log( 'Paused PayPal Secure subscription #{sub_id} for transaction {txn_id}.', ITE_Log_Levels::INFO, array(
		...
		it_exchange_log( 'Failed to pause PayPal Secure subscription #{sub_id} for transaction {txn_id}: {response}', array(
			'sub_id'   => $subscription->get_subscriber_id(),
			'txn_id'   => $subscription->get_transaction()->get_ID(),

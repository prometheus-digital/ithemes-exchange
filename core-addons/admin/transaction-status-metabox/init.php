...
class IT_Cart_Buddy_Core_Addon_Transaction_Status_Meta_Box {
	...
	*/
	function print_meta_box( $post ) {
		...
		if ( ! it_cart_buddy_add_on_supports( $transaction_method, 'transaction_statuses' ) )
			return;

		$available_statuses  = it_cart_buddy_get_add_on_support( $transaction_method, 'transaction_statuses' );
		...
		if ( count( $available_statuses ) < 2 ) {
			...
			foreach( $available_statuses as $slug => $name ) {
				...
				<label for="it_cart_buddy_transaction_status-<?php esc_attr_e( $slug ); ?>">
					<input type="radio" id="it_cart_buddy_transaction_status-<?php esc_attr_e( $slug ); ?>" name="transaction_status" <?php checked( $slug, $current_status ); ?> value="<?php esc_attr_e( $slug ); ?>" /> <?php esc_attr_e( $name ); ?><br />
				</label>
				<?php
			}
			?></div><?php
	...
	*/
	function update_transaction_status( $post ) {
		...
		if ( $transaction )
			it_cart_buddy_update_transaction_status( $transaction, $_POST['transaction_status'] );
	}
}
global $pagenow;

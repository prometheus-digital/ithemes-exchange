<?php
/**
 * This file registers the core Manual Payments Gateway
 *
...
/**
 * Core CartBuddy Manual Payments Gateway Class
 *
 * @since 0.1
*/
if ( ! class_exists( 'IT_Core_Manual_Payments_Gateway' ) ) {
	class IT_Core_Product_Manual_Payments_Gateway {
		 ...
		 * @since 0.1
		*/
		function IT_Core_Product_Manual_Payments_Gateway() {
			// Register Gatway
			...
			add_action( 'it_cb_product_gateways_manual_payments_after_mbs', array( $this, 'add_submit_button' ), 99, 2 );
		}
		...
		*/
		function register_gateway() {
            $options = array(
                'var'       => 'manual_payments',
                'title'     => __( 'Manual Payments', 'LION' ),
            );
            $options = apply_filters( 'it_cartbuddy_core_maual_payments_gateway_options', $options );
		...
		*/
		function hook_meta_boxes() {
			do_action( 'it_cartbuddy_core_before_hook_manual_payments_settings_mb' );
            add_meta_box(
                'it_cb_manual_payments_basic_settings',
                __( 'Basic Settings', 'LION' ),
                array( $this, 'print_basic_settings' ),
                'it_cb_product_gateways_manual_payments'
            );
			do_action( 'it_cartbuddy_core_after_hook_manual_payments_settings_mb' );
		}
		...
		*/
		function print_basic_settings( $form ) {
            $settings = cartbuddy( 'get_options', array( 'var' => 'it_cb_product_gateways_manual_payments' ) );
            if ( ! empty( $settings ) ) {
            ...
            <table class="form-table">
                <?php do_action( 'it_cartbuddy_before_core_manual_payments_basic_settings_rows', $form ); ?>
                <tr valign="top">
                ...
                <?php do_action( 'it_cartbuddy_after_core_manual_payments_basic_settings_rows', $form ); ?>
            </table>
            <?php
		}
		...
		*/
		function add_submit_button( $form ) {
			$form->add_submit( 'it_cartbuddy_submit_manual_payment_gateway_options' );
		}
	}
	new IT_Core_Product_Manual_Payments_Gateway();
}
